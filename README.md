...
FHIR R4 EOB → Layer 1 FHIR Parser (fhir_converter.py) → Layer 2 ChromaDB RAG (pos_rag_store.py, 66 CPT embeddings) → Layer 3 Rule Engine (fwa_data_pipeline.py, NCCI 131,611 pairs, MUE 12,453 limits, ICD-CPT 56 rules, PFS 9,222 rates) → Layer 4 Groq LLM (fwa_langchain_reasoning.py, llama-3.3-70b-versatile, 4.5s/claim) → Streamlit Dashboard (mediaguard_dashboard.py)
...
Month 2 — Package: In Progress  
Month 3 — Position: Upcoming
